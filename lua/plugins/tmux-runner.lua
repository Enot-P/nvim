  ...
  config = function()
    local function dart_hot_reload()
      ...
      if target_window then
        local cmd = string.format("tmux send-keys -t %s 'r' Enter", target_window)
        vim.fn.system(cmd)
      else
        -- Пробуем отправить в следующее окно
        local next_window = tonumber(current_window) + 1
        local cmd = string.format("tmux send-keys -t %s 'r' Enter 2>/dev/null", next_window)
        
        if vim.v.shell_error ~= 0 then
          -- Пробуем предыдущее окно
          local prev_window = tonumber(current_window) - 1
          if prev_window > 0 then
            cmd = string.format("tmux send-keys -t %s 'r' Enter", prev_window)
            vim.fn.system(cmd)
          end
        end
      end
    ...
    vim.keymap.set('n', '<leader>dr', dart_hot_reload, { desc = 'Dart hot reload' })
  end,
}
